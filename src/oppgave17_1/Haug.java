...
public final class Haug<T extends Comparable<? super T>> implements MaksHaugADT<T> {
    ...
    public Haug(int startkapasitet) {
     ...
     * bortsett fra roten  kanskje ikke oppyller kravet om å være
     * større enn barna sine. Metoden reparerer undertreet slik at undertreet
    ...
    public <T extends Comparable<? super T>> Haug<T> finnKMinste(T[] tabell, int k) {
        Haug<T> haug = new Haug<>(k);
        ...
        for (T verdi : tabell) {
            ...
            else if (verdi.compareTo(haug.finnMaks()) < 0) {
                // Ny verdi er mindre enn største i heapen → bytt ut
                haug.fjernMaks();
                haug.leggTil(verdi);
            }
        }
